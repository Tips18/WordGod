...
- 视觉系统：V1 前端采用以 `#E6DCCA`、`#FED8B6`、`#FFCFAB`、`#FFC2B0` 为主的暖色纸本文献风格。应用壳层使用普通文档流顶部标题栏，不随滚动吸顶，仅提供“阅读检测”和“生词本”两个可见页面入口；阅读页以完整段落面板为主，`1280px` 及以上宽度使用正文居中的三列布局，本篇已选列表作为左侧旁批轨按 lemma 去重展示已选词面并提供 `×` 移除按钮，Live Note 作为右侧固定旁批轨承载词汇详情；认证弹窗、生词本列表和详情页复用同一套纸张面板、细线分隔、暖色状态标签和主按钮样式。
...
- `vocabulary`：生词本列表和详情。
- `content`：白名单抓取、标准化、翻译、WordCram 文章自然段抽取、ECDICT 词典导入、OpenAI Batch 富化和入库命令。
...
- 题库导入流程会从英语一和英语二每篇 `Text 1-4` 抽取所有英文自然段，写入 `content-cache/wordcram-article-passages.json`，再通过 OpenAI Batch 生成句子翻译和 token 释义，最后以 `textIndex + paragraphIndex` 维度 upsert 到 PostgreSQL 的 `Passage` 与 `LexiconEntry`。
