...
后端使用 `NestJS + Prisma + PostgreSQL schema`。业务服务通过 `AppStore` 抽象访问数据，`WORD_GOD_STORE=prisma` 时使用 `PrismaAppStore` 连接 PostgreSQL，`WORD_GOD_STORE=memory` 时使用带考研英语种子段落的 `InMemoryAppStore` 便于无数据库演示和测试。
