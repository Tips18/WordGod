...
V1 前端采用“纸本文献感”的阅读优先设计。界面围绕 `#E6DCCA`、`#FED8B6`、`#FFCFAB`、`#FFC2B0` 四个暖色组织，首页突出完整真题段落，词汇详情以旁批式信息面板呈现。该视觉方向只服务现有阅读检测、登录认证和生词本流程，不扩展新的考试类型或内容后台。

顶部导航会先通过 `/auth/me` 恢复当前登录态；当 access Cookie 已失效但 refresh Cookie 仍有效时，后端会补发 access Cookie。游客显示“登录 / 注册”入口，已登录用户显示不可点击的“已登录”状态按钮。
